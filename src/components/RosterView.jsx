...
export default function RosterView({ activeGame }) {
    ...
    const [allOfficialUsers, setAllOfficialUsers] = useState([]);
    ...
    const fetchTeams = async () => {
        try {
            ...
            setTeams(res.data.filter(t => t.teamStatus === 'OFFICIAL'));
        } catch (error) {
    ...
    const fetchAllUsers = async () => {
        try {
            ...
            setAllOfficialUsers(res.data.filter(u => u.role === 'OFFICIAL'));
        } catch (error) {
    ...
    const handleCreateTeam = async () => {
        ...
        try {
            await api.post('/teams/create', { name: newTeamName, type: filter, teamStatus: 'OFFICIAL' });
        ...
        return (
            <div key={team._id} style={{ marginBottom: '2rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                    <h4 style={{ color: 'var(--accent-cyan)', textTransform: 'uppercase', letterSpacing: '1px', margin: 0 }}>
                        {team.name} ({team.type})
                    </h4>
                    <button onClick={() => setManagingTeam(team)} className="btn-primary" style={{ padding: '6px 12px', fontSize: '0.8rem', background: 'transparent', borderColor: 'var(--accent-cyan)', color: 'var(--accent-cyan)' }}>
                        Manage Details
                    </button>
                </div>
                ...
                {players.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {players.map((p, idx) => (
                            <div key={p._id || idx} className="glass-panel" style={{ padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderLeft: `4px solid var(--accent-cyan)` }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', width: '25%' }}>
                                    <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'var(--bg-card)', border: `2px solid var(--accent-cyan)` }}></div>
                                    <div>
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <div>
                    <h3 style={{ marginBottom: '0.25rem' }}>Official Roster ({activeGame})</h3>
                    <p className="text-secondary">View active squads and duos. Players can hold concurrent assignments.</p>
                </div>
            ...
            {filter === 'SQUAD' && (
                <>
                    <h3 style={{ color: 'var(--text-primary)', margin: '1rem 0 1rem', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '0.5rem' }}>Active Squad Formations</h3>
                    {filteredTeams.map(renderSection)}
                    {filteredTeams.length === 0 && <p className="text-muted">No official squads formed yet.</p>}
                </>
            )}

            {filter === 'DUO' && (
                <>
                    <h3 style={{ color: 'var(--text-primary)', margin: '1rem 0 1rem', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '0.5rem' }}>Active Duo Formations</h3>
                    {filteredTeams.map(renderSection)}
                    {filteredTeams.length === 0 && <p className="text-muted">No official duos formed yet.</p>}
                </>
            )}
            ...
            {showCreateModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                    <div className="glass-panel" style={{ width: '90%', maxWidth: '400px', padding: '2rem', position: 'relative', border: `1px solid var(--accent-cyan)` }}>
                        ...
                        <h3 style={{ marginBottom: '1.5rem', color: 'var(--accent-cyan)' }}>Create New Team</h3>
                        ...
                        <button onClick={handleCreateTeam} className="btn-primary" style={{ width: '100%' }}>Create {filter === 'SQUAD' ? 'Squad' : 'Duo'}</button>
                    </div>
                </div>
            )}
            ...
            {managingTeam && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                    <div className="glass-panel" style={{ width: '90%', maxWidth: '500px', padding: '2rem', position: 'relative', border: `1px solid var(--accent-cyan)` }}>
                        ...
                        <h3 style={{ marginBottom: '0.5rem', color: 'var(--accent-cyan)' }}>Manage: {managingTeam.name}</h3>
                        ...
                        <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '0.5rem' }}>
                            ...
                            >
                                <option value="">-- Select Official Player to Add --</option>
                                {allOfficialUsers
                            ...
                            <button onClick={handleAddMember} disabled={!selectedUserToAdd} className="btn-primary" style={{ padding: '0 1rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                <Plus size={16}/> Add
                            </button>
                        </div>
